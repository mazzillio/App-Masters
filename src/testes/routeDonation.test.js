...
describe("Teste da rota /donation do servidor", () => {
    ...
    it("Api deve retornar falha pela falta de devices", async() => {
        ...
        expect(response.body.error).toBe(true);
        expect(response.body).toHaveProperty("errorMessage");
        expect(response.body.errorMessage).toStrictEqual("Todos os campos obrigatórios devem ser informados:");
    ...
    it("Api deve retornar falha pela diferença entre devices passados e deviceCount", async() => {
        ...
        expect(response.body.error).toBe(true);
        expect(response.body).toHaveProperty("errorMessage");
        expect(response.body.errorMessage).toStrictEqual("A quantidade de equipamentos (2) não está de acordo com as informações de equipamentos enviados (1)");
        expect(response.status).toBe(400);
    });
    it("Api deve retornar falha por tipo de device invalido", async() => {
        ...
        expect(response.body.error).toBe(true);
        expect(response.body).toHaveProperty("errorMessage");
        expect(response.body.errorMessage).toStrictEqual("types dos equipamentos Computador estão incorretos");
        expect(response.status).toBe(400);
    });
